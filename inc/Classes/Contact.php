...
class Contact {
	...
	public function handle_contact_form() {
		...
		// Send emails
		$email_sent = $this->send_notification_email( $first_name, $last_name, $email, $phone, $subject, $message );
		$thank_you_sent = $this->send_thank_you_email( $first_name, $last_name, $email );
	 ...
	 * @return bool
	 */
	private function send_notification_email( string $first_name, string $last_name, string $email, string $phone, string $subject, string $message ): bool {
		$to      = 'info@lionselectsafaris.com';
		$subject_email = sprintf(
			/* translators: %s: Site name */
			__( '[%s] Nouveau message de contact', 'lunivers-theme' ),
			get_bloginfo( 'name' )
		);
		...
		$body = $this->get_email_template(
			sprintf(
				/* translators: %s: Site name */
				__( 'Nouveau message de contact depuis %s', 'lunivers-theme' ),
				get_bloginfo( 'name' )
			),
			sprintf(
				/* translators: %s: Site name */
				__( 'Vous avez reçu un nouveau message de contact depuis le site %s.', 'lunivers-theme' ),
				get_bloginfo( 'name' )
			),
			[
				[
	 ...
	 * @return bool
	 */
	private function send_thank_you_email( string $first_name, string $last_name, string $email ): bool {
		$to      = $email;
		$subject_email = sprintf(
			/* translators: %s: Site name */
		...
		$body = $this->get_email_template(
			sprintf(
				/* translators: %s: Site name */
				__( 'Merci pour votre message', 'lunivers-theme' ),
				get_bloginfo( 'name' )
			),
			sprintf(
